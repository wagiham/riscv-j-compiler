...
typedef struct token_st {
  token_type type;
  int32_t
      literal_value;  // this field used to store the value of literal tokens
...
//
// Arguments:
// - j_file: the file stream to read from. Should be a valid file and have read
// permissions.
// - output: the output parameter to "return" the read token. The type must
// always be set.
...
// Extra function which you may wish to define and use, but are not required to
// use
